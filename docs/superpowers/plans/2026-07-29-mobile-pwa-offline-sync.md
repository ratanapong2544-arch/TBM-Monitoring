...
**Keep everything else in both files**: they pin the mid-edit guards and App's mirror rules, which Task 7 needs and this task must not regress. Every listed test asserts the notice text or `Save to Cloud`'s `disabled` state, both of which this step removes, so leaving them in place fails the suite at Step 6 with no indication of which side is wrong.
