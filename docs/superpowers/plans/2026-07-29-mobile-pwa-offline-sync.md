...
repository.load(machine)
// -> Promise<{ data, source: "indexeddb" | "empty", fetchedAt, stale: true }>
// A cache read has NO `present`: a stored snapshot says nothing about what the server most recently
// sent. That is why any consumer of `present` must also require `source === "server"`.
...
**Delete, do not carry, Task 7's interim write bookkeeping** (added after Task 7's review). In `ShiftReportView.jsx`: the module-scope `shiftSaveState` map — a draft id, the set of ids known to be on the sheet, and a serialising chain, all keyed `machine|date|shift` — plus `SHIFT_SAVE_TIMEOUT_MS` and `withDeadline`, the "outcome unknown" block (`state.blocked` and its `state.blockSerial` epoch, surfaced as `saveUnresolved`), the `checkWithServer` handler with both its notices, the `checking`, `checkFailed` and `bumpUnresolved` state, the cold-launch gate (`snapshotReady` prop, `snapshotReadyRef`, `snapshotReadyAtSave`, `wouldAppend`/`appendBlocked`, the `SHIFT_SAVE_NO_SNAPSHOT` path), and `savingKeys`/`markSaving` (a per-report saving set exists only because a save could stall). In `App.jsx` — which is in this task's file list — the `onRefresh={offlineData.refresh}` and `snapshotReady={…}` props passed to `ShiftReportView`, whose only purpose is that check and the cold-launch gate. `repository.refresh`'s `serverPayload` field exists only for that check; remove it once nothing reads it. **Keep `refresh()`'s null-on-stale-machine contract** (`useOfflineData.js` returns `null` rather than another machine's payload when the machine changed under it) — this step removes both of its current consumers, and Task 10's manual-sync callers need that behaviour to survive the tidy-up. `normalizeServerData`'s `present` flags exist only for the cold-launch gate — check whether Task 9's reconciliation wants them (an absent collection is not an empty one there either) before removing.

Delete only the **tests of that bookkeeping**:

- in `shiftReportMidEdit.test.jsx` — the `describe("what may release an unknown outcome")` block, and the tests naming the deadline, the server check, the unknown outcome, and the snapshot gate (`a time bar cannot create a report before this machine's rows have arrived`, `an update is not refused while the snapshot is still loading`, `Save stays available for a report the sheet already has, with no snapshot`, `a queued save is judged on the snapshot state when the crew acted`);
- in `appDataFlow.test.jsx` — `a blocked shift report can be unblocked from the app itself`, `a cold launch cannot append a second report for a shift that already has one`, `a cached snapshot does not satisfy the create-a-report gate`, `a response that omits the shift reports does not open the create gate`, `a launch whose fetch failed can still be recovered without losing the typed report`, and the `__resetShiftSaveStateForTests()` call in its `beforeEach`. **Keep `the real repository hands App everything the create-a-report gate needs`** if any field still crosses that seam; otherwise keep its shape for whatever replaces `present` — three blockers in this task hid in repository→App and nothing else crosses it.

**Keep everything else in both files**: they pin the mid-edit guards and App's mirror rules, which Task 7 needs and this task must not regress.

**Delete the block and route the write in the same commit.** Between the two there is a window in which a timed-out, non-idempotent `addShiftReport` has no protection at all — the exact defect that cost this project rounds 15 through 24 of review.

**Assert sends, not notices, when you rewrite these tests.** Proven during review: against a source deliberately broken in the way one of these tests exists to catch, the notice assertion in that very test still passed — only the send count caught it. Every rule these tests encode is a rule about *what was sent*. When rewriting against the queue, count mutation envelopes per `domainKey`; a notice, a `disabled` attribute or a spinner proves nothing about what is in flight. And **run each rewritten test against a deliberately broken source before believing it** — three of this task's pins were vacuous, and all three were caught only that way, never by reading.

How to classify a test the list forgot: it goes **only** if it depends on something this step deletes — the notice text, `Save to Cloud`'s `disabled` state, `SHIFT_SAVE_TIMEOUT_MS`, the check button, or the unknown-outcome block. A test that merely asserts which action was sent (`addShiftReport` versus `updateShiftReport`) is about the queue's job and must be **rewritten against the queue**, not deleted — those are the duplicate-row rules that took this task fifteen rounds to get right.
