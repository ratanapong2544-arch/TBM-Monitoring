...
- **A cold launch could append a duplicate with a perfectly healthy server and no timeout at all.** On a cold cache the splash clears as soon as the empty cache pass settles — that is Task 7's whole point — so the Shift Report renders a **blank, editable** form for as long as the snapshot takes, up to the 90 s ceiling. A night crew filling in a shift the day crew had already saved would append a second row, silently, because `existingReport` was null. Appending is now gated on **a server answer for this machine this session** (`snapshotReady`), with a notice saying so and a button that fetches one. Nothing is lost offline: `apiCall` fails immediately without a connection anyway. **Online it is not always seconds:** on a link where the 463 KB snapshot times out at 90 s but a kilobyte write would land, a report that does not yet exist cannot be created until a fetch succeeds — the notice's button is the only retry. Updating an existing report is unaffected.
...
## Round 19 — the gate that locked the crew out
...
## Round 21 — the gate that was closed on every healthy device

Both reviewers found the same BLOCKER independently, and it was mine from the previous round. `normalizeServerData` attaches `present`, but `repository.refresh` returns what `writeServerSnapshot` gives back — and that function rebuilds its return value from `emptyServerData`, so `present` was dropped. The gate therefore read `undefined` **whenever IndexedDB was writable**, which is to say always: Save disabled, every time bar refused, and the recovery button calling the same `refresh` that produced the problem. It opened only when the cache was broken. Strictly worse than the round-19 blocker it descends from, because no fetch could clear it.

**All 847 tests passed against it.** Every App-level test used a fake repository whose fixture fabricated `present` — pinning the predicate's shape while the supply was invented. The seam that has now hidden three blockers is repository→App, and nothing crossed it. `appDataFlow.test.jsx` now drives `<App/>` through the **real** `createRepository` over real (fake-indexeddb) storage; reverting the carry-through fails it, and no fixture can satisfy it.
...
- **Files outside Task 7's list that were also modified**, for completeness: `src/offline/repository.js` (the `setSyncMetaValue` passthrough Task 7 Step 3 requires, the `cacheError` branch that keeps a server-fresh payload usable when the cache write fails, the guarded catch that stops an unopenable database masking a network fault, and the `serverPayload` field on `refresh` — the raw GAS response, which the shift report's server check must read instead of the merged snapshot; it is listed for deletion in Task 8 Step 4), `src/offline/mutationStore.js`, `src/offline/db.js` (Task 3's file — the `onblocked`/timeout fix), `src/offline/apiTransport.js` (Task 4's file — the snapshot timeout), `src/offline/syncRunner.js` (Task 5's file — the unhandled-rejection fix on its event trigger) and `src/offline/normalizeServerData.js` (Task 4's file — the `present` flags, which distinguish a collection the response omitted from one the server has none of). `src/offline/snapshotStore.test.js` was updated to state that `present` is deliberately not persisted. Five new test files were added outside the list as well.
...
- **`ShiftReportView` gained user-facing surface** — the server-copy notice with its two-step discard confirmation, a standing per-report notice when a save's outcome is unknown (with a button that re-reads the server), a cold-launch notice with its own fetch button, a disabled Save button while that gate holds, and four alerts: a timed-out save, a save refused because an earlier outcome is still unknown, a save refused because no server answer has arrived for this machine yet, and a save that landed after a machine switch. It follows from the mid-edit hazard Task 7 creates and matches the design's "never silently overwrite" posture, but the discard/confirm pattern overlaps what Task 10's Sync Center will own. **Task 10 should absorb it, not duplicate it.**
...
- **Deletion propagation — now owned by Task 9 Step 5b.** An empty server collection no longer clears local state (`App.jsx`: `if (data.issues.length)`, `if (data.machineProgress)`, `mirrorInst`'s `!rows.length`), so deleting the last issue on device B leaves device A showing it forever, labelled server-confirmed. The trade is deliberate — `normalizeServerData` maps an absent key to `[]`, so an older GAS deployment or a partial `doGet` is indistinguishable from a real deletion, and losing a field record outranks showing a stale one — so reconciliation is the first point that can tell the two apart. Task 9 Step 5b now carries it, with the distinguishing mechanism to be named there (a per-collection revision, an explicit count, or tombstones).
