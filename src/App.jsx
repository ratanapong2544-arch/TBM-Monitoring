...
const formatDisplayDate = (d) => {
  ...
  if (typeof d === "string" && d.includes("T")) {
    ...
    if (!isNaN(dateObj.getTime())) {
      // ใช้ en-CA เพื่อบังคับ Format ให้ออกมาเป็น YYYY-MM-DD พร้อมบีบ Timezone ให้เป็น Asia/Bangkok
  ...
  return (
    <g onClick={onClick} style={{ cursor: "pointer" }}>
      ...
      <text
        ...
        transform={`rotate(${-midAngle + midAngle}, ${labelPos.x}, ${
          labelPos.y
        })`}
      >
        {label}
    ...
    <div className="relative flex flex-col items-center justify-center p-4">
      ...
      >
        ...
        <g
          ...
          className="transition-transform duration-700 cubic-bezier(0.34, 1.56, 0.64, 1)"
        >
          {segments.map((seg) => (
            <RingSegment
              key={seg.id}
              ...
              endAngle={seg.end}
              label={seg.label}
              isSelected={selectedPositions[seg.id]}
              onClick={() => onTogglePosition(seg.id)}
            />
          ))}
          <circle cx={cx} cy={cy} r="6" fill="#3B82F6" className="shadow-md" />
          <path
            d={`M${cx},${cy - r + 25} L${cx},${cy - r + 45}`}
            stroke="#3B82F6"
            strokeWidth="3"
            strokeLinecap="round"
          />
...
const OverviewView = ({
  ...
  const liveStatus = useMemo(() => {
    if (segmentRecords.length === 0)
      return { state: "IDLE", ring: "-", desc: "ยังไม่มีข้อมูล" };
    ...
    if (lastSeg.status === "In Progress") {
      if (lastSeg.excavStartTime && !lastSeg.excavEndTime) {
        return {
          state: "EXCAVATING",
          ring: lastSeg.ringNo,
          desc: `เริ่มขุดเมื่อ ${formatDisplayTime(lastSeg.excavStartTime)} น.`,
          color: "amber",
        };
        ...
        return {
          ...
          ring: lastSeg.ringNo,
          desc: `รอติดตั้ง Segment (ขุดเสร็จ ${formatDisplayTime(
            lastSeg.excavEndTime
          )} น.)`,
          color: "slate",
        };
        ...
        return {
          state: "INSTALLING",
          ring: lastSeg.ringNo,
          desc: `เริ่มประกอบเมื่อ ${formatDisplayTime(
            lastSeg.installStartTime
          )} น.`,
          color: "emerald",
        };
      }
      return {
        state: "IN_PROGRESS",
        ring: lastSeg.ringNo,
        desc: "กำลังดำเนินการบันทึกข้อมูล...",
        color: "blue",
      };
    } else {
      return {
        state: "IDLE",
        ring: offsetRingNo(lastSeg.ringNo, 1),
        desc: "รอเริ่มขุดวงถัดไป",
        color: "slate",
      };
    }
  }, [segmentRecords]);
  ...
  const groutStatus = useMemo(() => {
    ...
    const completedSegs = segmentRecords.filter(
      (s) => s.status === "Completed"
    );
    const latestSeg =
      completedSegs.length > 0
        ? completedSegs[completedSegs.length - 1].ringNo
        : "-";

    const latestGrout =
      groutRecords.length > 0
        ? groutRecords[groutRecords.length - 1].ringNo
        : "-";
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-24 animate-fade-in">
      <div
        className={`rounded-[2rem] p-6 sm:p-8 text-white relative overflow-hidden shadow-xl ${
          liveStatus.state === "EXCAVATING"
            ? "bg-gradient-to-br from-amber-500 to-orange-600 shadow-orange-500/30"
            : liveStatus.state === "INSTALLING"
            ? "bg-gradient-to-br from-emerald-500 to-teal-600 shadow-emerald-500/30"
            : liveStatus.state === "WAITING_INSTALL"
      ...
      >
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs sm:text-sm font-bold uppercase tracking-widest opacity-90">
          ...
          <div className="flex justify-between items-start mb-4">
            ...
            <div className="text-right">
              <div className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest">
                วงล่าสุดที่ติดตั้งเสร็จ
              </div>
              <div className="text-xl sm:text-2xl font-black text-slate-800">
                {groutStatus.latestSeg}
              </div>
            </div>
          </div>
          <p className="text-xs sm:text-sm text-slate-500 mb-4">
            เข้าสู่หน้าบันทึกข้อมูลเวลาการขุดเจาะและการประกอบ Segment แบบละเอียด
          </p>
...
const GroutRecordView = ({
  ...
  const isReGrout = useMemo(() => {
    ...
    return groutRecords.some(
      (r) => r.ringNo.toUpperCase() === formData.ringNo.toUpperCase()
    );
  ...
  useEffect(() => {
    if (
      ...
      const nextGroutRing = getRingByOffsetFromHistory(
        lastRecord.ringNo,
        2,
        segmentRecords
      );
      const latestSegmentRing =
        segmentRecords.length > 0
          ? segmentRecords[segmentRecords.length - 1].ringNo
          : "";
      setFormData((prev) => ({
        ...prev,
        ringNo: nextGroutRing,
        excavRing: latestSegmentRing,
      }));
    }
  ...
  useEffect(() => {
    if (formData.ringNo) {
      const segment = segmentRecords.find(
        (s) => s.ringNo.toUpperCase() === formData.ringNo.toUpperCase()
      );
      if (segment) {
        setFormData((prev) =>
          prev.keyType !== segment.keyPos
            ? { ...prev, keyType: segment.keyPos }
            : prev
        );
        setIsKeyLinked(true);
  ...
  const currentTotal = (
    parseFloat(formData.partA || 0) + parseFloat(formData.partB || 0)
  ).toFixed(2);
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "ringNo") {
      newFormData.excavRing = getRingByOffsetFromHistory(
        value,
        3,
        segmentRecords
      );
  ...
  const togglePosition = (pos) =>
    setFormData((prev) => ({
      ...prev,
      positions: { ...prev.positions, [pos]: !prev.positions[pos] },
    }));
  ...
  const handleSubmit = async (e) => {
    ...
    const existingRecord = groutRecords.find(
      (r) => r.ringNo.toUpperCase() === formData.ringNo.toUpperCase()
    );
    ...
    if (existingRecord) {
      ...
      if (!currentRemark.includes("ครั้งที่ 1")) {
        currentRemark = `ครั้งที่ 1 วันที่ ${formatDisplayDate(
          existingRecord.date
        )} ปริมาณ ${parseFloat(existingRecord.total).toFixed(
          2
        )}\n${currentRemark}`.trim();
      }
      ...
      let appendedRemark = `ครั้งที่ ${nextPass} วันที่ ${formatDisplayDate(
        projectInfo.date
      )} ปริมาณ ${currentTotal}`;
      if (formData.remark) {
      ...
      const newPartA =
        parseFloat(existingRecord.partA || 0) + parseFloat(formData.partA || 0);
      const newPartB =
        parseFloat(existingRecord.partB || 0) + parseFloat(formData.partB || 0);
      ...
      const updatedRecord = {
        ...existingRecord,
        partA: newPartA.toFixed(2),
        partB: newPartB.toFixed(2),
      ...
      try {
        ...
        setGroutRecords((prev) =>
          prev.map((r) => (r.id === updatedRecord.id ? updatedRecord : r))
        );
        resetFormAfterSave(true);
      } catch (err) {
        alert("อัปเดตข้อมูลไม่สำเร็จ กรุณาลองใหม่: " + err.message);
      }
    } else {
      const newRecord = {
        id: `grout_${Date.now()}`,
  ...
  const resetFormAfterSave = (wasReGrout) => {
    setFormData((prev) => {
      const nextGroutRing = !wasReGrout
        ? getRingByOffsetFromHistory(prev.ringNo, 2, segmentRecords)
        : prev.ringNo;
      const latestSegmentRing = !wasReGrout
        ? segmentRecords.length > 0
          ? segmentRecords[segmentRecords.length - 1].ringNo
          : ""
        : prev.excavRing;
          ...
          <div className="relative z-10 flex justify-between items-center">
            <div>
              ...
              <p className="text-blue-100 text-xs sm:text-sm mt-1 opacity-90">
                Enter details for the primary grout
              </p>
            </div>
            <div className="text-right">
        ...
        <div className="p-4 sm:p-8 space-y-4 sm:space-y-6">
          <div className="grid grid-cols-2 gap-3 sm:gap-4">
            <div className="bg-slate-50 p-3 sm:p-4 rounded-2xl border border-slate-100 focus-within:border-blue-400 focus-within:ring-4 focus-within:ring-blue-50 transition-all duration-300 relative">
              <label className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">
                Working Date
              </label>
              <input
                type="date"
                ...
                className="w-full bg-transparent text-base sm:text-lg font-black text-slate-800 outline-none cursor-pointer"
              />
            </div>
            <div className="bg-slate-50 p-3 sm:p-4 rounded-2xl border border-slate-100 focus-within:border-blue-400 focus-within:ring-4 focus-within:ring-blue-50 transition-all duration-300 relative">
              <label className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4">
            <div className="bg-slate-50 p-3 sm:p-4 rounded-2xl border border-slate-100 focus-within:border-blue-400 focus-within:ring-4 focus-within:ring-blue-50 transition-all duration-300 relative">
              <div className="flex justify-between items-center mb-1">
                <label className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest block">
                  Grouting Ring
                </label>
                <span
              ...
              <input
                ...
                className="w-full bg-transparent text-xl sm:text-2xl font-black text-slate-800 placeholder-slate-300 outline-none uppercase mt-1"
                placeholder="P-XXXX"
              />
              {isReGrout && (
                <p className="text-[8px] sm:text-[9px] text-purple-600 font-bold mt-1">
                  ⚠️ เคย Grout แล้ว: บันทึกปริมาณเฉพาะส่วนที่ฉีดเพิ่ม
                </p>
              )}
            </div>
            <div className="bg-slate-50 p-3 sm:p-4 rounded-2xl border border-slate-100 focus-within:border-blue-400 focus-within:ring-4 focus-within:ring-blue-50 transition-all duration-300 relative flex flex-col justify-end">
              <label className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 flex justify-between items-center">
                ...
                <Activity
                  ...
                  title="Auto filled from latest segment"
                />
              </label>
              <input
                type="text"
                name="excavRing"
                value={formData.excavRing}
                onChange={handleInputChange}
                className="w-full bg-transparent text-xl sm:text-2xl font-black text-slate-800 placeholder-slate-300 outline-none uppercase mt-1"
                placeholder="P-XXXX"
              />
            ...
            <div className="relative z-10">
              <div className="flex justify-between items-center mb-3">
                <span className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  Key Segment
                  {isKeyLinked && (
                    <span className="bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-[4px] text-[8px] sm:text-[9px] flex items-center gap-1">
                      <Check size={10} /> Synced
                    </span>
                  )}
                </span>
                <span
                  className={`text-base sm:text-lg font-black px-3 py-1 rounded-lg border ${
                    isKeyLinked
              ...
              <input
                ...
                onChange={handleInputChange}
                disabled={isKeyLinked}
                className={`w-full h-3 rounded-full appearance-none accent-blue-600 ${
                  isKeyLinked
          ...
          <div className="bg-slate-900 rounded-[1.5rem] sm:rounded-[2rem] p-4 sm:p-6 text-white shadow-2xl relative overflow-hidden">
            ...
            <div className="relative z-10 grid grid-cols-3 gap-2 sm:gap-3 mb-4 sm:mb-6">
              <div className="bg-white/5 hover:bg-white/10 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-white/10 transition-colors focus-within:bg-white/10 focus-within:border-white/30">
                <label className="text-slate-400 text-[9px] sm:text-[10px] font-bold uppercase tracking-widest block mb-1">
                  Part A
                </label>
                <div className="flex items-baseline gap-1">
                  <input
                    ...
                    onChange={handleInputChange}
                    className="w-full bg-transparent text-lg sm:text-xl font-bold text-white placeholder-white/20 outline-none p-0 border-none focus:ring-0"
                    placeholder="0.00"
                  />
            ...
            <div className="relative z-10 flex items-end justify-between bg-black/20 p-4 sm:p-5 rounded-2xl border border-white/5">
              <div>
                <span className="text-slate-400 text-[9px] sm:text-[10px] font-bold uppercase tracking-widest block mb-1">
                  Total Volume
                </span>
              ...
              <div className="text-right">
                ...
                <div
                  className={`text-3xl sm:text-4xl font-black ${
                    currentRatio > 150
                      ? "text-purple-400 drop-shadow-[0_0_12px_rgba(168,85,247,0.3)]"
                      : currentRatio >= 100
          ...
          <div className="bg-white p-4 border border-slate-200 rounded-2xl shadow-sm">
            <label className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block">
              Problem / Remark (ปัญหา/อุปสรรค)
            </label>
            <textarea
              name="remark"
              ...
              onChange={handleInputChange}
              rows="2"
              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-50 transition-all mb-3"
              placeholder="ไม่มี..."
            ></textarea>
            <div className="border-t border-slate-100 pt-3">
              <label className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-1">
                <ImageIcon size={12} /> Attach Photo (บันทึกรูปลง Drive)
              </label>
              <input
                type="file"
                accept="image/*"
                onChange={(e) => handleFileUpload(e, setFormData)}
                className="text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 transition-all cursor-pointer w-full"
              />
            </div>
          </div>
          ...
          <button
            type="submit"
            disabled={isSaving}
            className={`w-full text-white font-bold text-base sm:text-lg py-4 sm:py-5 rounded-2xl shadow-xl transition-all active:scale-[0.98] flex items-center justify-center gap-2 ${
              isSaving
                ? "bg-slate-400 cursor-not-allowed shadow-none"
                : "bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 shadow-blue-500/30"
            }`}
          >
            {isSaving ? (
              <>
              ...
              <>
                <Save size={24} /> Confirm & Save Record
              </>
            )}
          </button>
        </div>
    ...
    if (segmentRecords.length > 0 && !formData.ringNo) {
      ...
      if (lastRecord.status === "In Progress") {
        setFormData((prev) => ({
          ...
          installStartTime:
            lastRecord.installStartTime || lastRecord.startTime || "",
    ...
    try {
      if (formData.id) {
        ...
        setSegmentRecords((prev) =>
          prev.map((r) => (r.id === recordData.id ? recordData : r))
        );
      } else {
      ...
      setFormData((prev) => {
        ...
        return {
          ...
          finishCH: isCompleted
            ? formatCH(parseCH(prev.finishCH) - parseFloat(prev.length))
            : prev.finishCH,
          remark: "",
      ...
      >
        <div className="bg-gradient-to-br from-teal-600 to-emerald-700 px-6 sm:px-8 py-5 sm:py-6 text-white relative overflow-hidden">
          ...
          <div className="relative z-10 flex justify-between items-center">
            <div>
              <h2 className="font-extrabold text-xl sm:text-2xl flex items-center gap-2">
                <Layers size={24} /> Segment Install
              </h2>
              <p className="text-emerald-100 text-xs sm:text-sm mt-1 opacity-90">
                Record daily segment installation (Save partially allowed)
              </p>
            </div>
            <div className="text-right">
              <div className="text-[10px] sm:text-xs bg-white/20 backdrop-blur-md px-3 py-1.5 rounded-xl border border-white/20 font-medium">
        ...
        <div className="p-4 sm:p-8 space-y-4 sm:space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4">
            <div className="bg-slate-50 p-3 sm:p-4 rounded-2xl border border-slate-100 focus-within:border-emerald-400 focus-within:ring-4 focus-within:ring-emerald-50 transition-all">
              ...
              <input
                ...
                value={formData.ringNo}
                onChange={handleInputChange}
                className="w-full bg-transparent text-xl sm:text-2xl font-black text-slate-800 outline-none uppercase"
                placeholder="P-XXXX"
              />
            </div>
            <div className="bg-slate-50 p-3 sm:p-4 rounded-2xl border border-slate-100 focus-within:border-emerald-400 focus-within:ring-4 focus-within:ring-emerald-50 transition-all">
              ...
              <select
                ...
                onChange={handleInputChange}
                className={`w-full bg-transparent text-lg sm:text-xl font-black outline-none cursor-pointer ${
                  formData.installType === "Temporary"
                    ? "text-amber-600"
                    : "text-emerald-700"
                }`}
              >
                <option value="Permanent" className="text-slate-800">
                  ถาวร (Permanent)
                </option>
                <option value="Temporary" className="text-slate-800">
                  ชั่วคราว (Temporary)
                </option>
              </select>
            </div>
          ...
          <div className="bg-white border border-slate-200 rounded-[1.5rem] p-4 sm:p-5 shadow-sm space-y-4 sm:space-y-5 relative overflow-hidden">
            <div className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest border-l-2 border-amber-500 pl-2">
              Excavation Phase (ขุดเจาะ)
            </div>
            <div className="grid grid-cols-2 gap-4 sm:gap-6 relative z-10 mt-2">
              <div>
                <label className="text-[10px] sm:text-xs font-bold text-slate-500 flex items-center gap-1.5 mb-2">
                  <PlayCircle size={12} className="text-amber-500" /> Start
                </label>
                <input
                  type="time"
                  name="excavStartTime"
                  value={formData.excavStartTime}
                  onChange={handleInputChange}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base text-slate-800 font-bold outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-50 transition-all"
                />
              </div>
              <div>
                <label className="text-[10px] sm:text-xs font-bold text-slate-500 flex items-center gap-1.5 mb-2">
                  <CheckCircle2 size={12} className="text-amber-500" /> Finish
                </label>
                <input
                  type="time"
                  name="excavEndTime"
                  value={formData.excavEndTime}
                  onChange={handleInputChange}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base text-slate-800 font-bold outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-50 transition-all"
                />
              </div>
            ...
            <div className="mt-3 bg-slate-50 border border-slate-200 rounded-xl p-3 sm:p-4 focus-within:border-amber-500 focus-within:ring-1 focus-within:ring-amber-50 transition-all relative z-10">
              <label className="text-[10px] sm:text-xs font-bold text-slate-500 mb-1.5 block">
                ลักษณะของดิน (Soil Type)
              </label>
              <input
                type="text"
                name="soilType"
                ...
                onChange={handleInputChange}
                className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 outline-none mb-3"
                placeholder="เช่น ดินเหนียวปนทราย, Soft Clay..."
              />

              <div className="border-t border-slate-100 pt-3">
                <label className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-1">
                  <ImageIcon size={12} /> ภาพถ่ายดินขุด (ไม่บังคับ)
                </label>
                <input
                  type="file"
                  accept="image/*"
            ...
            <div className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest border-l-2 border-emerald-500 pl-2">
              Installation Phase (ประกอบ)
            </div>

            <div className="grid grid-cols-2 gap-4 sm:gap-6 relative z-10 mt-2">
              <div>
                <label className="text-[10px] sm:text-xs font-bold text-slate-500 flex items-center gap-1.5 mb-2">
                  <PlayCircle size={12} className="text-emerald-500" /> Start
                </label>
                <input
                  type="time"
                  name="installStartTime"
                  value={formData.installStartTime}
                  onChange={handleInputChange}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base text-slate-800 font-bold outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-50 transition-all"
                />
              </div>
              <div>
                <label className="text-[10px] sm:text-xs font-bold text-slate-500 flex items-center gap-1.5 mb-2">
                  <CheckCircle2 size={12} className="text-emerald-500" /> Finish
                </label>
                <input
                  type="time"
                  name="installEndTime"
              ...
              <div className="flex flex-col justify-center">
                <div className="flex justify-between items-center mb-1 sm:mb-2">
                  ...
                  <select
                    ...
                    onChange={handleInputChange}
                    className={`text-[9px] font-bold rounded px-1 outline-none cursor-pointer ${
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-base sm:text-lg font-bold outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-50 transition-all text-center"
                />

                <div className="mt-2 bg-emerald-50 rounded-lg p-1.5 text-center border border-emerald-100">
                  <span className="text-[9px] sm:text-[10px] text-emerald-600 font-bold uppercase tracking-widest block">
                    Soil Vol. (ดินขุด)
                  </span>
                  <span className="text-sm sm:text-base font-black text-emerald-700">
                    {calculateSoilVolume(formData.length)} m³
                  </span>
                </div>
              </div>
              <div className="flex flex-col justify-center">
                <div className="flex justify-between items-center mb-2 sm:mb-3">
                  ...
                  <span className="text-base sm:text-lg font-black text-emerald-600 bg-emerald-50 border border-emerald-100 px-2 py-0.5 sm:px-3 sm:py-1 rounded-lg">
                    K{formData.keyPos}
                  </span>
                </div>
                <input
                  ...
                  step="1"
                  name="keyPos"
                  value={formData.keyPos}
                  onChange={handleInputChange}
                  className="w-full h-2 sm:h-3 bg-slate-200 rounded-full appearance-none cursor-pointer accent-emerald-600"
                />
                ...
                <select
                  name="typeRing"
                  value={formData.typeRing}
                  onChange={handleInputChange}
                  className="mt-3 w-full bg-slate-50 border border-slate-200 rounded-lg px-2 py-1 text-sm font-bold text-slate-700 outline-none"
                >
          ...
          <div className="bg-slate-900 rounded-[1.5rem] sm:rounded-[2rem] p-4 sm:p-6 text-white shadow-xl relative overflow-hidden">
            ...
            <div className="relative z-10 flex items-center justify-between gap-2 sm:gap-4">
              <div className="flex-1">
                <label className="text-emerald-400 text-[9px] sm:text-[10px] font-bold uppercase tracking-widest block mb-1">
                  Start (CH.)
                </label>
                <input
                  type="text"
                  ...
                  onBlur={(e) =>
                    setFormData((prev) => ({
                      ...prev,
                      startCH: formatCH(prev.startCH),
                    }))
                  }
                  className="w-full bg-white/10 border border-white/10 rounded-xl px-2 sm:px-4 py-2 sm:py-3 text-lg sm:text-2xl font-black text-white outline-none focus:bg-white/20 transition-all text-center"
                  placeholder="8+815.68"
                />
              </div>
              ...
              <div className="flex-1">
                ...
                <input
                  type="text"
                  name="finishCH"
                  value={formData.finishCH}
                  onChange={handleInputChange}
                  onBlur={(e) =>
                    setFormData((prev) => ({
                      ...prev,
                      finishCH: formatCH(prev.finishCH),
                    }))
                  }
                  className="w-full bg-white/10 border border-white/10 rounded-xl px-2 sm:px-4 py-2 sm:py-3 text-lg sm:text-2xl font-black text-white outline-none focus:bg-white/20 transition-all text-center"
                  placeholder="8+814.28"
                />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-4 border border-slate-200 rounded-2xl shadow-sm">
            ...
            <textarea
              ...
              onChange={handleInputChange}
              rows="2"
              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-50 transition-all mb-3"
              placeholder="ไม่มี..."
            ></textarea>
          </div>
          ...
          <button
            type="submit"
            disabled={isSaving}
            className={`w-full text-white font-bold text-base sm:text-lg py-4 sm:py-5 rounded-2xl shadow-xl transition-all transform active:scale-[0.98] flex items-center justify-center gap-2 ${
              isSaving
                ? "bg-slate-400 cursor-not-allowed shadow-none"
                : "bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-700 shadow-emerald-500/30"
            }`}
          >
            {isSaving ? (
              <>
...
const GroutDashboardView = ({
  ...
  const chartData = useMemo(() => {
    let baseData =
      filterShift === "All"
        ? groutRecords
        : groutRecords.filter((r) => r.shift === filterShift);
    ...
    if (filterMode === "all") {
      ...
      return baseData.filter((r) =>
        formatDisplayDate(r.date).startsWith(filterMonth)
      );
    } else {
  ...
  useEffect(() => {
    if (isEditing && editFormData?.ringNo) {
      const segment = segmentRecords.find(
        (s) => s.ringNo.toUpperCase() === editFormData.ringNo.toUpperCase()
      );
      if (segment) {
        setEditFormData((prev) =>
          prev.key !== segment.keyPos ? { ...prev, key: segment.keyPos } : prev
        );
  ...
  const handleEditChange = (e) => {
    ...
    setEditFormData((prev) => {
      ...
      if (name === "ringNo") {
        updated.excavRing = getRingByOffsetFromHistory(
          value,
          3,
          segmentRecords
        );
  ...
  const handleSaveEdit = async () => {
    setIsSaving(true);
    const total =
      parseFloat(editFormData.partA || 0) + parseFloat(editFormData.partB || 0);
  ...
  const editTotal = isEditing
    ? (
        parseFloat(editFormData.partA || 0) +
        parseFloat(editFormData.partB || 0)
      ).toFixed(2)
    : 0;
  const editRatio = isEditing
    ? ((editTotal / THEORETICAL_VOL) * 100).toFixed(1)
    : 0;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8 animate-fade-in pb-24">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <StatCard
          label="Unique Rings"
          value={new Set(groutRecords.map((r) => r.ringNo)).size}
          subtext="Records found"
          color="text-blue-600"
          icon={FileText}
        />
        <StatCard
          label="Avg. Volume"
          value={(
            groutRecords.reduce((acc, r) => acc + r.total, 0) /
            (groutRecords.length || 1)
          ).toFixed(2)}
          subtext="Cubic Meters"
        ...
        <StatCard
          label="Avg. Ratio"
          value={`${(
            groutRecords.reduce((acc, r) => acc + r.ratio, 0) /
            (groutRecords.length || 1)
          ).toFixed(1)}%`}
          subtext="Efficiency"
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 items-start sm:items-center w-full lg:w-auto bg-slate-50 p-2 rounded-xl border border-slate-100">
            <div className="flex bg-white rounded-lg p-1 border border-slate-200 shadow-sm w-full sm:w-auto overflow-x-auto">
              <button
                ...
                className={`flex-1 sm:flex-none px-3 py-1.5 text-xs rounded-md font-bold transition whitespace-nowrap ${
                  filterMode === "all"
            ...
            {filterMode === "lastN" && (
              <div className="flex gap-1 w-full sm:w-auto overflow-x-auto">
                {[10, 20, 50, 100].map((val) => (
                  <button
                    key={val}
                    onClick={() => setChartWindow(val)}
                    className={`px-3 py-1.5 text-xs rounded-md font-medium border transition whitespace-nowrap ${
                      chartWindow === val
            ...
            {filterMode === "daily" && (
              <input
                ...
                className="px-3 py-1.5 text-xs font-bold border border-slate-200 rounded-lg focus:ring-1 focus:ring-blue-500 outline-none text-slate-700 w-full sm:w-auto"
              />
            )}
            {filterMode === "monthly" && (
              <input
            ...
            {filterMode === "range" && (
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <div className="relative flex-1 sm:flex-none">
                  <span className="absolute left-2 top-1.5 text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase">
                    From
                  </span>
                  <input
                    type="text"
                    ...
                    placeholder="P1"
                    className="pl-2 pt-4 pb-1 w-full sm:w-20 text-xs font-bold border rounded-lg focus:ring-1 focus:ring-blue-500 outline-none uppercase text-slate-700"
                  />
                </div>
            ...
            <select
              ...
              className="px-3 py-1.5 text-xs font-bold border border-slate-200 rounded-lg focus:ring-1 focus:ring-blue-500 outline-none text-slate-700 bg-white cursor-pointer w-full sm:w-auto"
            >
        ...
        <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-100 flex flex-col sm:flex-row justify-between sm:items-center bg-slate-50/50 gap-2">
          ...
          <div className="flex gap-2 self-end sm:self-auto">
            <button
              ...
              className="p-1.5 border rounded-md bg-white hover:bg-slate-50"
            >
              <ChevronLeft size={14} />
            </button>
            <span className="px-3 py-1.5 text-[10px] sm:text-xs border rounded-md bg-white">
            ...
            <tbody className="divide-y divide-slate-50">
              {tableData.map((record, index) => (
                ...
                >
                  <td className="px-4 sm:px-6 py-3 sm:py-4">
                    <div className="flex items-center gap-2">
                      ...
                      {record.groutPass === "Re-Grout" && (
                        <span className="bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded-[4px] text-[8px] sm:text-[9px] font-bold">
                          Re-Grout
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      ...
                      ></span>
                      <span className="text-[10px] sm:text-xs text-slate-500 font-medium">
                        {formatDisplayDate(record.date)} ({record.shift})
                      </span>
                    </div>
                  </td>
                  <td className="px-4 sm:px-6 py-3 sm:py-4">
                    <div className="text-[10px] sm:text-xs font-mono bg-slate-100 px-2 py-0.5 rounded text-slate-600 inline-block mb-1">
                      Key {record.key}
                    </div>
                    <div className="flex gap-1 flex-wrap">
                      {Object.entries(record.positions || {}).map(
                        ([pos, active]) =>
                          active && (
                            <span
                              key={pos}
                              className="px-1.5 py-0.5 bg-blue-50 text-blue-700 text-[9px] sm:text-[10px] rounded border border-blue-100 font-bold"
                            >
                              {pos}
                            </span>
                          )
                  ...
                  <td className="px-4 sm:px-6 py-3 sm:py-4 text-right">
                    <span
                      className={`inline-flex px-2 py-1 rounded-full text-[10px] sm:text-xs font-bold border ${
                        record.ratio > 150
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh] sm:max-h-[90vh] transform transition-all">
            <div className="bg-gradient-to-r from-blue-600 to-indigo-700 px-4 sm:px-6 py-3 sm:py-4 text-white flex justify-between items-center shrink-0">
              <div>
                <h3 className="font-bold text-base sm:text-lg flex items-center gap-2">
                  ...
                  {isEditing ? "Edit Ring Data" : "Ring Details"}
                </h3>
              ...
              <div className="flex items-center gap-1 sm:gap-2">
                {!isEditing && (
                  <>
                    {selectedRecord.imageUrl &&
                      selectedRecord.imageUrl !== "Attached" && (
                        <a
                    ...
                    <button
                      ...
                      className="p-1.5 sm:p-2 bg-white/10 hover:bg-red-500 rounded-full transition-colors"
                      title="Delete"
                    >
                ...
                <button
                  ...
                  className="p-1.5 sm:p-2 bg-white/10 hover:bg-white/20 rounded-full transition-colors ml-1 sm:ml-2"
                >
            ...
            {showDeleteConfirm && (
              <div className="bg-red-50 p-3 sm:p-4 flex justify-between items-center border-b border-red-100 shrink-0 animate-fade-in">
                <span className="text-red-700 text-xs sm:text-sm font-bold flex items-center gap-2">
                  <Trash2 size={14} className="sm:w-[16px] sm:h-[16px]" />{" "}
                  ยืนยันการลบข้อมูล Ring {selectedRecord.ringNo} ใช่หรือไม่?
                </span>
                <div className="flex gap-2">
                  <button
                    onClick={() => setShowDeleteConfirm(false)}
                    className="px-3 sm:px-4 py-1.5 bg-white text-slate-600 rounded-lg shadow-sm text-[10px] sm:text-xs font-bold border border-slate-200 hover:bg-slate-100"
                  >
                    ยกเลิก
                  </button>
                  <button
                    ...
                    disabled={isSaving}
                    className="px-3 sm:px-4 py-1.5 bg-red-600 text-white rounded-lg shadow-sm text-[10px] sm:text-xs font-bold hover:bg-red-700 flex items-center gap-1"
                  >
            ...
            <div className="p-4 sm:p-6 overflow-y-auto space-y-4 sm:space-y-6">
              <div className="flex flex-wrap justify-between items-center bg-slate-50 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100 gap-2">
                <div>
                  <div className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">
                    Grouting Ring
                  </div>
                  {isEditing ? (
                    <div className="flex items-center gap-2">
                      <input
                        name="ringNo"
                        value={editFormData.ringNo}
                        onChange={handleEditChange}
                        className="text-xl sm:text-2xl font-black text-blue-600 bg-white border border-blue-200 rounded-lg px-2 py-1 w-24 sm:w-32 outline-none focus:ring-2 focus:ring-blue-500 uppercase"
                      />
                      <select
                        name="groutPass"
                        value={editFormData.groutPass || "1st Pass"}
                        onChange={handleEditChange}
                        className="border border-slate-300 rounded px-1 py-1 outline-none focus:border-blue-500 text-[10px] sm:text-xs font-bold text-slate-600"
                      >
                  ...
                  <div className="text-[10px] sm:text-xs text-slate-500 mt-1 flex items-center gap-1">
                    ...
                    {isEditing ? (
                      <input
                        name="excavRing"
                        value={editFormData.excavRing}
                        onChange={handleEditChange}
                        className="border border-slate-300 rounded px-1 py-0.5 outline-none focus:border-blue-500 w-16 uppercase"
                      />
                    ) : (
                      selectedRecord.excavRing
                    )}
                  </div>
                </div>
                <div className="text-right">
                  ...
                  {isEditing ? (
                    <div className="flex flex-col items-end gap-1 mt-1">
                      <input
                        type="date"
                        name="date"
                        value={formatDisplayDate(editFormData.date)}
                        onChange={handleEditChange}
                        className="border rounded px-2 py-1 text-xs sm:text-sm outline-none"
                      />
                      <select
                        name="shift"
                    ...
                    <>
                      <div className="text-xs sm:text-sm font-bold text-slate-700 flex items-center justify-end gap-1">
                        <Calendar
                          size={12}
                      ...
                      <div className="text-[10px] sm:text-xs text-slate-500 mt-1 flex items-center justify-end gap-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                <div
                  className={`bg-slate-50 rounded-xl sm:rounded-2xl border ${
                    isEditing
                      ? "border-blue-300 shadow-inner"
                      : "border-slate-100"
                  } p-3 sm:p-4 flex flex-col items-center justify-center relative`}
                >
                  <span className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 text-center">
                    Injection Configuration
                  </span>
                  {isEditing && (
                    <span className="absolute top-2 left-2 sm:top-3 sm:left-3 bg-blue-100 text-blue-700 text-[9px] sm:text-[10px] font-bold px-2 py-1 rounded">
                      EDIT MODE
                    </span>
                  )}
                  <div className="scale-75 sm:scale-90 transform origin-top -mt-2 sm:-mt-4">
                    <RingVisualizer
                      ringKey={
                        isEditing ? editFormData.key : selectedRecord.key
                      }
                      selectedPositions={
                        isEditing
                          ? editFormData.positions
                          : selectedRecord.positions
                      }
                      onTogglePosition={
                        isEditing ? handleEditPosition : () => {}
                      }
                    />
                  </div>
                  {isEditing && (
                    <div className="w-full mt-0 sm:mt-2 px-2">
                      <label className="text-[9px] sm:text-[10px] font-bold text-slate-400 flex justify-between items-center mb-1">
                        <span className="flex items-center gap-1 sm:gap-2">
                          KEY (1-16){" "}
                          {isEditKeyLinked && (
                            <span className="bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-[4px] text-[7px] sm:text-[8px] flex items-center gap-1">
                              <Check size={8} /> Synced
                            </span>
                          )}
                        </span>
                        <span
                          className={`text-xs sm:text-sm font-black px-2 py-0.5 rounded border ${
                            isEditKeyLinked
                              ? "text-emerald-700 bg-emerald-50 border-emerald-200"
                              : "text-blue-600 bg-blue-50 border-blue-100"
                          }`}
                      ...
                      <input
                        ...
                        onChange={handleEditChange}
                        disabled={isEditKeyLinked}
                        className={`w-full h-2 rounded-lg appearance-none accent-blue-600 ${
                          isEditKeyLinked
                            ? "bg-emerald-200 cursor-not-allowed opacity-70"
                ...
                <div className="space-y-3 sm:space-y-4 flex flex-col justify-center">
                  <div className="bg-blue-50 rounded-xl p-3 sm:p-4 border border-blue-100">
                    <div className="text-[10px] sm:text-xs font-bold text-blue-500 uppercase tracking-widest mb-3">
                      Volume Data
                    </div>
                      ...
                      <div>
                        ...
                        {isEditing ? (
                          <input
                            ...
                            step="0.01"
                            name="partA"
                            value={editFormData.partA}
                            onChange={handleEditChange}
                            className="w-full bg-white border border-blue-300 rounded-lg px-2 py-1 text-center font-mono text-base sm:text-lg font-bold text-blue-700 outline-none focus:ring-2 focus:ring-blue-500 shadow-inner"
                          />
                        ) : (
                          <div className="font-mono text-base sm:text-lg font-bold text-slate-700">
                            {selectedRecord.partA}{" "}
                      ...
                      <div>
                        ...
                        {isEditing ? (
                          <input
                            ...
                            step="0.01"
                            name="partB"
                            value={editFormData.partB}
                            onChange={handleEditChange}
                            className="w-full bg-white border border-blue-300 rounded-lg px-2 py-1 text-center font-mono text-base sm:text-lg font-bold text-blue-700 outline-none focus:ring-2 focus:ring-blue-500 shadow-inner"
                          />
                        ) : (
                          <div className="font-mono text-base sm:text-lg font-bold text-slate-700">
                            {selectedRecord.partB}{" "}
                            <span className="text-[10px] sm:text-xs font-sans text-slate-400">
                              m³
                            </span>
                          </div>
                    ...
                    <div className="border-t border-blue-200/50 pt-2 sm:pt-3 flex justify-between items-end">
                      <div>
                        ...
                        <div className="text-xl sm:text-2xl font-black text-blue-700">
                          {isEditing
                            ? editTotal
                            : selectedRecord.total.toFixed(2)}{" "}
                          <span className="text-xs sm:text-sm font-sans font-normal text-blue-500">
                            m³
                          </span>
                        </div>
                      </div>
                      <div className="text-right">
                        ...
                        <div
                          className={`text-lg sm:text-xl font-black ${
                            (isEditing ? editRatio : selectedRecord.ratio) > 150
                              ? "text-purple-500"
                              : (isEditing
                                  ? editRatio
                                  : selectedRecord.ratio) >= 100
                              ? "text-emerald-500"
                              : "text-red-500"
                          }`}
                        >
                          {isEditing
                            ? editRatio
                            : selectedRecord.ratio.toFixed(1)}
                          %
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 sm:gap-4">
                    <div className="bg-white rounded-xl p-3 sm:p-4 border border-slate-200 shadow-sm">
                      ...
                      {isEditing ? (
                        <input
                          ...
                          onChange={handleEditChange}
                          className="w-full bg-slate-50 border border-slate-300 rounded-lg px-2 py-1 text-left font-mono text-base sm:text-lg font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500"
                        />
                      ) : (
                        <div className="font-mono text-lg sm:text-xl font-bold text-slate-700">
                          {selectedRecord.pressure}{" "}
                          <span className="text-xs sm:text-sm font-sans text-slate-400 font-normal">
                            bar
                          </span>
                        </div>
                      )}
                    ...
                    <div className="bg-white rounded-xl p-3 sm:p-4 border border-slate-200 shadow-sm flex flex-col justify-center">
                      ...
                      <div className="mt-1">
                        <span
                          className={`inline-flex items-center px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-bold border ${
                            (isEditing ? editRatio : selectedRecord.ratio) > 150
                              ? "bg-purple-50 text-purple-700 border-purple-200"
                              : (isEditing
                                  ? editRatio
                        ...
                        >
                          {(isEditing ? editRatio : selectedRecord.ratio) > 150
                            ? "over design"
                            : (isEditing ? editRatio : selectedRecord.ratio) >=
                              100
                            ? "Pass"
                            : "below design"}
                        </span>
              ...
              {(selectedRecord.remark || isEditing) && (
                <div className="bg-orange-50/50 rounded-xl p-3 sm:p-4 border border-orange-100">
                  <div className="text-[10px] sm:text-xs font-bold text-orange-500 uppercase tracking-widest mb-2 flex items-center gap-1">
                  ...
                  {isEditing ? (
                    <textarea
                      ...
                      onChange={handleEditChange}
                      className="w-full bg-white border border-orange-200 rounded-lg p-2 text-xs sm:text-sm text-slate-700 outline-none focus:ring-2 focus:ring-orange-400"
                      rows="2"
            ...
            {isEditing && (
              <div className="bg-slate-50 p-3 sm:p-4 border-t border-slate-100 flex justify-end gap-2 sm:gap-3 shrink-0">
                <button
                  ...
                  className="px-4 sm:px-6 py-2 sm:py-2.5 rounded-xl font-bold text-xs sm:text-sm text-slate-500 hover:bg-slate-200 transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={isSaving}
                  className="px-4 sm:px-6 py-2 sm:py-2.5 rounded-xl font-bold text-xs sm:text-sm bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-200 transition-all active:scale-95 flex items-center gap-1 sm:gap-2"
                >
                  {isSaving ? (
                    <Loader2
                      size={14}
...
const SegmentDashboardView = ({ segmentRecords, setSegmentRecords }) => {
    ...
    try {
      ...
      if (saved) {
        ...
        if (parsed.dailyPlan !== undefined) {
          return {
            defaultDailyPlan: parsed.dailyPlan,
            basePlanAcc: parsed.basePlanAcc || 0,
            baseActualAcc: parsed.baseActualAcc || 0,
            planOverrides: [],
          };
        }
        return parsed;
  ...
  const getPlanForDate = (dateStr, config) => {
    if (!config.planOverrides || config.planOverrides.length === 0)
      return parseFloat(config.defaultDailyPlan) || 0;
    const sorted = [...config.planOverrides].sort(
  ...
  const baseSegmentRecords = useMemo(() => {
    return filterShift === "All"
      ...
      : deduplicatedSegments.filter((r) => r.shift === filterShift);
  ...
  const handleSaveEdit = async () => {
    ...
    const updatedRecord = {
      ...editFormData,
      soilVolume: calculateSoilVolume(editFormData.length),
    };
    try {
  ...
  const stats = useMemo(() => {
    const completedSegments = baseSegmentRecords.filter(
      (s) => s.status !== "In Progress"
    );
    const permSegments = completedSegments.filter(
      (s) => s.installType !== "Temporary"
    );
    const tempSegments = completedSegments.filter(
      (s) => s.installType === "Temporary"
    );
    ...
    const totalDistance = permSegments.reduce(
      (acc, rec) => acc + parseFloat(rec.length || 0),
      0
    );
    const totalSoilVol = permSegments.reduce(
      (acc, rec) =>
        acc + parseFloat(rec.soilVolume || calculateSoilVolume(rec.length)),
      0
    );

    const uniqueDays =
      new Set(baseSegmentRecords.map((r) => formatDisplayDate(r.date))).size ||
      1;
    const avgRings = (permRings / uniqueDays).toFixed(1);
    const avgDist = (totalDistance / uniqueDays).toFixed(2);
    const currentCH =
      baseSegmentRecords.length > 0
        ? baseSegmentRecords[baseSegmentRecords.length - 1].finishCH
        : "-";
  ...
  const fullDailyProgress = useMemo(() => {
    ...
    let sortedArray = Object.values(dateMap).sort(
    ...
    sortedArray = sortedArray.map((day) => {
      ...
      return {
        ...
        displayDate: new Date(day.date).toLocaleDateString("th-TH", {
          day: "numeric",
          month: "short",
        }),
  ...
  const chartData = useMemo(() => {
    if (filterMode === "daily") {
      const dayRecords = baseSegmentRecords.filter(
        (r) => formatDisplayDate(r.date) === filterDate
      );

      let baselineAcc = parseFloat(planConfig.baseActualAcc) || 0;
      const sortedDates = [
        ...new Set(baseSegmentRecords.map((r) => formatDisplayDate(r.date))),
      ].sort();
      for (const d of sortedDates) {
        if (d < filterDate) {
          baselineAcc += baseSegmentRecords.filter(
            (r) =>
              formatDisplayDate(r.date) === d &&
      ...
      if (sortedDates.length > 0 && filterDate > sortedDates[0]) {
        ...
        while (tempD < endD) {
          baselinePlan += getPlanForDate(
            tempD.toISOString().split("T")[0],
            planConfig
          );
      ...
      const hourlyData = Array.from({ length: 24 }, (_, i) => ({
        displayDate: `${String(i).padStart(2, "0")}:00`,
        dayRings: 0,
        nightRings: 0,
        tempRings: 0,
        totalRings: 0,
        plan: currentDayPlan / 24,
      }));
      ...
      return hourlyData.map((h) => {
        ...
        return {
          ...h,
          actualAcc: currentAcc,
          planAcc: Math.round(currentPlan * 10) / 10,
        };
      });
    }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8 animate-fade-in pb-24">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <StatCard
          label="Permanent Rings"
          value={stats.permRings}
          subtext={`+ ${stats.tempRings} Temp. (Total: ${stats.totalRings})`}
          color="text-blue-600"
          icon={Layers}
        />
        <StatCard
          label="Perm. Distance"
          value={`${stats.totalDistance.toFixed(2)} m`}
          subtext={`ดินขุดรวม: ${stats.totalSoilVol.toFixed(2)} m³`}
          color="text-emerald-600"
          icon={TrendingUp}
        />
        <StatCard
          label="Daily Average"
          value={`${stats.avgRings} Rings`}
          subtext={`~ ${stats.avgDist} m / day`}
          color="text-orange-500"
          icon={Activity}
        />
        <StatCard
          label="Current Position"
          value={stats.currentCH}
          subtext="Latest Finish CH."
      ...
      <div className="bg-white p-4 sm:p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-lg sm:text-xl font-extrabold text-slate-800 flex items-center gap-2">
            <TrendingUp className="text-emerald-600 sm:w-6 sm:h-6 w-5 h-5" />{" "}
            Ring Progress Monthly Report
          </h2>
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            TBM1 Segment Installation Tracking
          </p>
        </div>
        <div className="flex gap-2 sm:gap-4 text-[10px] sm:text-xs font-bold bg-slate-50 p-2 sm:p-3 rounded-xl border border-slate-100 w-full md:w-auto justify-between sm:justify-start">
          <div className="flex flex-col items-center">
            <span className="w-3 h-3 sm:w-4 sm:h-4 rounded bg-yellow-300 mb-1"></span>
            Day Shift
          </div>
          <div className="flex flex-col items-center">
            <span className="w-3 h-3 sm:w-4 sm:h-4 rounded bg-blue-500 mb-1"></span>
            Night Shift
          </div>
          {filterMode !== "daily" && (
            <>
              <div className="flex flex-col items-center">
                <span className="w-3 sm:w-4 h-0.5 bg-black mt-1.5 sm:mt-2 mb-1 sm:mb-1.5"></span>
                Plan Acc.
              </div>
              <div className="flex flex-col items-center">
                <span className="w-3 sm:w-4 h-0.5 bg-red-500 mt-1.5 sm:mt-2 mb-1 sm:mb-1.5"></span>
                Actual Acc.
              </div>
            </>
          )}
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-4 sm:mb-6 gap-3 sm:gap-4">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-1.5 text-slate-400 hover:text-emerald-600 bg-slate-50 hover:bg-emerald-50 rounded-lg transition-colors border border-slate-100"
              title="ตั้งค่าแผนงาน (Plan Settings)"
            >
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 items-start sm:items-center w-full lg:w-auto bg-slate-50 p-2 rounded-xl border border-slate-100">
            ...
            {filterMode === "daily" && (
              <input
                ...
                value={filterDate}
                onChange={(e) => setFilterDate(e.target.value)}
                className="px-3 py-1.5 text-xs font-bold border border-slate-200 rounded-lg focus:ring-1 focus:ring-emerald-500 outline-none text-slate-700 w-full sm:w-auto"
              />
            )}
            {filterMode === "monthly" && (
              <input
            ...
            {filterMode === "range" && (
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <input
                  ...
                  value={rangeStart}
                  onChange={(e) => setRangeStart(e.target.value)}
                  className="px-2 py-1.5 flex-1 sm:flex-none sm:w-[110px] text-[10px] sm:text-xs font-bold border border-slate-200 rounded-lg focus:ring-1 focus:ring-emerald-500 outline-none text-slate-700"
                />
                <span className="text-slate-400">-</span>
                <input
                  type="date"
            ...
            <select
              value={filterShift}
              onChange={(e) => setFilterShift(e.target.value)}
              className="px-3 py-1.5 text-xs font-bold border border-slate-200 rounded-lg focus:ring-1 focus:ring-emerald-500 outline-none text-slate-700 bg-white cursor-pointer w-full sm:w-auto"
            >
              <option value="All">All Shifts</option>
              <option value="Day">Day Shift</option>
              <option value="Night">Night Shift</option>
          ...
          <div className="min-w-[700px] h-full">
            <ResponsiveContainer>
              ...
              >
                ...
                <YAxis
                  yAxisId="left"
                  domain={[0, filterMode === "daily" ? "auto" : 10]}
                  tick={{ fontSize: 10, fill: "#94A3B8" }}
                  axisLine={false}
                  tickLine={false}
                />
                ...
                {filterMode !== "daily" && (
                  <Line
                    ...
                    stroke="#94a3b8"
                    strokeWidth={2}
                    dot={chartData.length <= 24 ? { r: 0 } : { r: 2 }}
                    name="Plan Daily"
                  />
                )}
                <Bar
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredTableRecords.length > 0 ? (
                  ...
                  >
                    <td className="px-4 sm:px-6 py-3 sm:py-4">
                      ...
                      <div className="text-[9px] sm:text-[10px] text-slate-500 mt-1 font-mono">
                        <span className="font-bold">Excav:</span>{" "}
                        {formatDisplayTime(rec.excavStartTime)} -{" "}
                        {formatDisplayTime(rec.excavEndTime)}
                      </div>
                      <div className="text-[9px] sm:text-[10px] text-slate-500 mt-0.5 font-mono">
                        <span className="font-bold">Inst:</span>{" "}
                        {formatDisplayTime(
                          rec.installStartTime || rec.startTime
                        )}{" "}
                        - {formatDisplayTime(rec.installEndTime || rec.endTime)}
                      </div>
                      <div className="text-[10px] sm:text-xs text-slate-500 mt-1">
                        {rec.shift} Shift
                      </div>
                    ...
                    <td className="px-4 sm:px-6 py-3 sm:py-4">
                      ...
                      {rec.installType === "Temporary" && (
                        <div className="text-[9px] sm:text-[10px] text-amber-500 font-bold mt-0.5">
                          Temporary
                        </div>
                      )}
                      {rec.status === "In Progress" && (
                        <div className="text-[9px] sm:text-[10px] text-orange-500 font-bold mt-0.5">
                          In Progress
                        </div>
                      )}
                    </td>
                    <td className="px-4 sm:px-6 py-3 sm:py-4 text-center">
                      <span className="bg-slate-100 px-2 py-0.5 sm:py-1 rounded text-[10px] sm:text-xs font-bold text-slate-600 mr-1 sm:mr-2">
                        {rec.typeRing}
                      </span>
                    ...
                    <td className="px-4 sm:px-6 py-3 sm:py-4 text-right font-mono text-slate-600">
                      {rec.startCH}
                    </td>
                    <td className="px-4 sm:px-6 py-3 sm:py-4 text-right font-mono text-slate-800 font-bold">
                      {rec.finishCH}
                    </td>
                    <td className="px-4 sm:px-6 py-3 sm:py-4 text-right">
                      ...
                      <div className="text-[9px] sm:text-[10px] text-amber-600 font-bold mt-0.5">
                        {rec.soilVolume || calculateSoilVolume(rec.length)} m³
                      </div>
                    </td>
                  </tr>
                ))
                ...
                <tr>
                  ...
                  >
                    ไม่พบข้อมูล หรือกำลังรอเชื่อมต่อฐานข้อมูล...
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col transform transition-all max-h-[90vh]">
            <div className="bg-slate-100 px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-200 flex justify-between items-center shrink-0">
              <h3 className="font-bold text-slate-800 flex items-center gap-2 text-sm sm:text-base">
                <Settings size={16} className="text-slate-500" /> Plan Settings
              </h3>
              <button
            ...
            <div className="p-4 sm:p-6 space-y-3 sm:space-y-4 overflow-y-auto">
              <div>
                <label className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-widest block mb-1">
                  Default Rings/Day
                </label>
                <input
                  type="number"
                  step="0.1"
                  value={tempPlan.defaultDailyPlan}
                  onChange={(e) =>
                    setTempPlan({
                      ...tempPlan,
                      defaultDailyPlan: e.target.value,
                    })
                  }
                  className="w-full border border-slate-300 rounded-xl px-3 sm:px-4 py-2 font-bold text-slate-800 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-50"
                />
              </div>
              <div>
                <label className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-widest block mb-1">
                  Initial Plan Accumulation
                </label>
                <input
                  type="number"
                  value={tempPlan.basePlanAcc}
                  onChange={(e) =>
                    setTempPlan({ ...tempPlan, basePlanAcc: e.target.value })
                  }
                  className="w-full border border-slate-300 rounded-xl px-3 sm:px-4 py-2 font-bold text-slate-800 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-50"
                />
                <p className="text-[9px] sm:text-[10px] text-slate-400 mt-1">
                  ตั้งค่าเริ่มต้นของเส้น Plan สะสม
                </p>
              </div>
              <div>
                <label className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-widest block mb-1">
                  Initial Actual Accumulation
                </label>
                <input
                  type="number"
                  value={tempPlan.baseActualAcc}
                  onChange={(e) =>
                    setTempPlan({ ...tempPlan, baseActualAcc: e.target.value })
                  }
                  className="w-full border border-slate-300 rounded-xl px-3 sm:px-4 py-2 font-bold text-slate-800 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-50"
                />
                <p className="text-[9px] sm:text-[10px] text-slate-400 mt-1">
                  ตั้งค่ายกมาของยอดติดตั้งจริง
                </p>
              </div>

              <div className="border-t border-slate-100 pt-4">
                <label className="text-[10px] sm:text-xs font-bold text-slate-800 uppercase tracking-widest block mb-2">
                  Plan Overrides (ปรับแผนตามช่วงเวลา)
                </label>
                {tempPlan.planOverrides &&
                  tempPlan.planOverrides.map((ov, idx) => (
                    <div
                      key={idx}
                      className="flex items-center gap-2 mb-2 bg-slate-50 p-2 rounded-lg border border-slate-200"
                    >
                      ...
                      <button
                        onClick={() =>
                          setTempPlan((prev) => ({
                            ...prev,
                            planOverrides: prev.planOverrides.filter(
                              (_, i) => i !== idx
                            ),
                          }))
                        }
                        className="p-1 text-red-400 hover:text-red-600 bg-white rounded shadow-sm border border-slate-200"
                      >
                        <Trash2 size={12} />
                      </button>
                    </div>
                ...
                <div className="flex gap-2 items-end mt-2">
                  <div className="flex-1">
                    ...
                    <input
                      ...
                      className="w-full border border-slate-300 rounded-lg px-2 py-1.5 text-xs outline-none focus:border-emerald-500"
                    />
                  </div>
                  <div className="w-20">
                    <label className="text-[9px] text-slate-400 mb-1 block">
                    ...
                    <input
                      type="number"
                      step="0.1"
                      value={newOverride.plan}
                      onChange={(e) =>
                        setNewOverride({ ...newOverride, plan: e.target.value })
                      }
                      className="w-full border border-slate-300 rounded-lg px-2 py-1.5 text-xs outline-none focus:border-emerald-500"
                    />
                  </div>
                  <button
                    onClick={() => {
                      if (newOverride.date && newOverride.plan) {
                        setTempPlan((prev) => ({
                          ...prev,
                          planOverrides: [
                            ...(prev.planOverrides || []),
                        ...
                        setNewOverride({ date: "", plan: 4 });
                      }
                    }}
                    className="bg-slate-800 text-white px-3 py-1.5 rounded-lg text-xs font-bold mb-0.5 hover:bg-slate-700 transition-colors"
                  >
                    Add
                  </button>
                </div>
            ...
            <div className="bg-slate-50 p-3 sm:p-4 border-t border-slate-100 flex justify-end gap-2 shrink-0">
              <button
                onClick={() => setShowPlanModal(false)}
                className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg font-bold text-xs sm:text-sm text-slate-500 hover:bg-slate-200 transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={handleSavePlan}
                className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg font-bold text-xs sm:text-sm bg-emerald-600 hover:bg-emerald-700 text-white transition-colors"
              >
                Save
              </button>
            </div>
            ...
            <div className="bg-gradient-to-r from-teal-600 to-emerald-700 px-4 sm:px-6 py-3 sm:py-4 text-white flex justify-between items-center shrink-0">
              <div>
                <h3 className="font-bold text-base sm:text-lg flex items-center gap-2">
                  ...
                  {isEditing ? "Edit Segment Data" : "Segment Details"}
                </h3>
            ...
            <div className="p-4 sm:p-6 overflow-y-auto space-y-4 sm:space-y-6">
                    ...
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          value={editFormData.ringNo}
                          onChange={handleEditChange}
                          className="text-xl sm:text-2xl font-black text-emerald-600 bg-white border border-emerald-200 rounded-lg px-2 py-1 w-24 sm:w-32 outline-none focus:ring-2 focus:ring-emerald-500 uppercase"
                        />
                        <select
                          name="typeRing"
                          value={editFormData.typeRing}
                          onChange={handleEditChange}
                          className="border border-slate-300 rounded-lg px-2 py-1 outline-none focus:border-emerald-500 font-bold text-slate-700 text-sm sm:text-base"
                        >
                          <option value="C1">C1</option>
                          <option value="ST1">ST1</option>
                          <option value="ST2">ST2</option>
                      ...
                      <div className="flex items-center gap-2">
                        <select
                          name="installType"
                          value={editFormData.installType || "Permanent"}
                          onChange={handleEditChange}
                          className={`border border-slate-300 rounded px-2 py-1 outline-none text-[10px] sm:text-xs font-bold ${
                            editFormData.installType === "Temporary"
                              ? "text-amber-600"
                              : "text-emerald-700"
                          }`}
                        >
                          <option value="Permanent">Permanent (ถาวร)</option>
                        ...
                        <select
                          ...
                          onChange={handleEditChange}
                          className="border border-slate-300 rounded px-2 py-1 outline-none focus:border-emerald-500 text-[10px] sm:text-xs font-bold text-slate-600"
                        >
                          <option value="Completed">Completed</option>
                          <option value="In Progress">In Progress</option>
                        </select>
                      ...
                      <div className="flex items-center gap-2">
                        <div
                          className={`text-xl sm:text-2xl font-black ${
                            selectedRecord.installType === "Temporary"
                              ? "text-amber-600"
                              : "text-slate-800"
                          }`}
                        >
                        ...
                        <span className="bg-slate-200 px-2 py-1 rounded text-xs sm:text-sm font-bold text-slate-600">
                          {selectedRecord.typeRing}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 mt-1">
                        {selectedRecord.installType === "Temporary" && (
                          <span className="bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-[4px] text-[8px] sm:text-[9px] font-bold">
                            Temporary Ring
                          </span>
                        )}
                        {selectedRecord.status === "In Progress" && (
                          <span className="bg-orange-100 text-orange-700 px-1.5 py-0.5 rounded-[4px] text-[8px] sm:text-[9px] font-bold">
                            In Progress
                          </span>
                        )}
                      </div>
                  ...
                  {isEditing ? (
                    <div className="flex flex-col items-end gap-1 mt-1">
                      <input
                        ...
                        value={formatDisplayDate(editFormData.date)}
                        onChange={handleEditChange}
                        className="border rounded px-2 py-1 text-xs sm:text-sm outline-none focus:border-emerald-500"
                      />
                      <select
                        name="shift"
                        value={editFormData.shift}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                ...
                >
                  ...
                  {isEditing && (
                    <span className="absolute top-2 left-2 sm:top-3 sm:left-3 bg-emerald-100 text-emerald-700 text-[9px] sm:text-[10px] font-bold px-2 py-1 rounded">
                      EDIT MODE
                    </span>
                  )}
                  <div className="scale-75 sm:scale-90 transform origin-top -mt-2 sm:-mt-4">
                    <RingVisualizer
                      ringKey={
                        isEditing ? editFormData.keyPos : selectedRecord.keyPos
                      }
                      selectedPositions={{ K: true }}
                      onTogglePosition={() => {}}
                    />
                  </div>
                  {isEditing && (
                    <div className="w-full mt-0 sm:mt-2 px-2">
                      <label className="text-[9px] sm:text-[10px] font-bold text-slate-400 flex justify-between items-center mb-1">
                        ...
                        <span
                          className={`text-xs sm:text-sm font-black px-2 py-0.5 rounded border text-emerald-600 bg-emerald-50 border-emerald-100`}
                        >
                          K{editFormData.keyPos}
                        </span>
                      </label>
                      <input
                        type="range"
                        ...
                        step="1"
                        name="keyPos"
                        value={editFormData.keyPos}
                        onChange={handleEditChange}
                        className="w-full h-2 rounded-lg appearance-none accent-emerald-600 bg-slate-300 cursor-pointer"
                      />
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-3 sm:space-y-4 flex flex-col justify-center">
                  <div className="bg-emerald-50 rounded-xl p-3 sm:p-4 border border-emerald-100">
                    <div className="text-[10px] sm:text-xs font-bold text-emerald-600 uppercase tracking-widest mb-3">
                      Chainage Data
                    </div>
                      ...
                      <div>
                        ...
                        {isEditing ? (
                          <input
                            ...
                            className="w-full bg-white border border-emerald-300 rounded-lg px-2 py-1 text-left font-mono text-xs sm:text-sm font-bold text-emerald-700 outline-none focus:ring-2 focus:ring-emerald-500 shadow-inner"
                          />
                        ) : (
                          <div className="font-mono text-sm sm:text-base font-bold text-slate-700">
                            {selectedRecord.startCH}
                          </div>
                        )}
                      </div>
                      <div>
                        ...
                        <div className="flex flex-col items-end gap-1">
                          <div className="flex items-center gap-1">
                            <input
                              ...
                              onChange={handleEditChange}
                              className="w-16 sm:w-20 bg-white border border-emerald-300 rounded-lg px-2 py-1 text-right font-mono text-base sm:text-lg font-black text-emerald-700 outline-none focus:ring-2 focus:ring-emerald-500 shadow-inner"
                            />
                  ...
                  <div className="bg-white rounded-xl p-3 sm:p-4 border border-slate-200 shadow-sm flex flex-col gap-3">
                    <div className="flex justify-between items-center gap-2">
                      <div className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest w-16">
                        Excavate
                      </div>
                        ...
                        {isEditing ? (
                          <>
                            <input
                              type="time"
                              name="excavStartTime"
                              value={editFormData.excavStartTime || ""}
                              onChange={handleEditChange}
                              className="w-full bg-slate-50 border border-slate-300 rounded-lg px-2 py-1 text-center font-mono text-xs sm:text-sm font-bold text-slate-700 outline-none focus:border-emerald-500 focus:ring-1"
                            />
                            <span className="text-slate-300">-</span>
                            <input
                              type="time"
                              name="excavEndTime"
                              value={editFormData.excavEndTime || ""}
                              onChange={handleEditChange}
                              className="w-full bg-slate-50 border border-slate-300 rounded-lg px-2 py-1 text-center font-mono text-xs sm:text-sm font-bold text-slate-700 outline-none focus:border-emerald-500 focus:ring-1"
                            />
                          </>
                          ...
                          <div className="font-mono text-xs sm:text-sm font-bold text-slate-700">
                            {formatDisplayTime(selectedRecord.excavStartTime)} -{" "}
                            {formatDisplayTime(selectedRecord.excavEndTime)}
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center gap-2">
                      ...
                      <div className="flex-1 flex items-center justify-end gap-2">
                        {isEditing ? (
                          ...
                          <div className="font-mono text-xs sm:text-sm font-bold text-slate-700">
                            {formatDisplayTime(
                              selectedRecord.installStartTime ||
                                selectedRecord.startTime
                            )}{" "}
                            -{" "}
                            {formatDisplayTime(
                              selectedRecord.installEndTime ||
                ...
                <div className="bg-orange-50/50 rounded-xl p-3 sm:p-4 border border-orange-100">
                  ...
                  {isEditing ? (
                    <textarea
                      ...
                      value={editFormData.remark}
                      onChange={handleEditChange}
                      className="w-full bg-white border border-orange-200 rounded-lg p-2 text-xs sm:text-sm text-slate-700 outline-none focus:ring-2 focus:ring-emerald-500"
                      rows="2"
                      placeholder="เพิ่มหมายเหตุ..."
                    />
                  ) : (
            ...
            {isEditing && (
              <div className="bg-slate-50 p-3 sm:p-4 border-t border-slate-100 flex justify-end gap-2 sm:gap-3 shrink-0">
                ...
                <button
                  onClick={handleSaveEdit}
                  disabled={isSaving}
                  className="px-4 sm:px-6 py-2 sm:py-2.5 rounded-xl font-bold text-xs sm:text-sm bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg shadow-emerald-200 transition-all active:scale-95 flex items-center gap-1 sm:gap-2"
                >
                  {isSaving ? (
                    <Loader2
                      size={14}
...
const ReportView = ({ segmentRecords, groutRecords, projectInfo }) => {
  ...
  const filteredSegments = useMemo(() => {
    return deduplicatedSegments.filter((r) => {
      ...
      if (reportType === "monthly" && !dDate.startsWith(reportMonth))
        return false;
      return true;
    });
  ...
  const summary = useMemo(() => {
    const completedSegments = filteredSegments.filter(
      (s) => s.status !== "In Progress"
    );
    const permSegments = completedSegments.filter(
      (s) => s.installType !== "Temporary"
    ...
    const totalLength = permSegments
      .reduce((sum, s) => sum + parseFloat(s.length || 0), 0)
      .toFixed(2);
    const totalSoilVol = permSegments
      .reduce(
        (sum, s) =>
          sum + parseFloat(s.soilVolume || calculateSoilVolume(s.length)),
        0
      )
      .toFixed(2);
    const totalGroutVol = filteredGrouts
    ...
    const avgGroutRatio =
      filteredGrouts.length > 0
        ? (
            filteredGrouts.reduce(
              (sum, g) => sum + parseFloat(g.ratio || 0),
              0
            ) / filteredGrouts.length
          ).toFixed(1)
        : "0.0";
    const uniqueGroutedRings = new Set(filteredGrouts.map((g) => g.ringNo))
      .size;

    const allRemarks = [
      ...filteredSegments
        .filter((s) => s.remark)
        .map((s) => ({ ring: s.ringNo, module: "Segment", text: s.remark })),
      ...filteredGrouts
        .filter((g) => g.remark)
        .map((g) => ({ ring: g.ringNo, module: "Grout", text: g.remark })),
    ];
  ...
  const displayDateStr =
    reportType === "daily"
      ...
      : new Date(reportMonth + "-01").toLocaleDateString("en-GB", {
          month: "long",
          year: "numeric",
        });
      ...
      <div className="no-print bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-4 w-full md:w-auto">
          <select
            ...
            className="px-3 py-2 border border-slate-200 rounded-lg text-sm font-bold text-slate-700 outline-none focus:border-blue-500 bg-white cursor-pointer w-full sm:w-auto"
          >
            <option value="All">All Shifts</option>
            <option value="Day">Day Shift</option>
            <option value="Night">Night Shift</option>
          ...
          {reportType === "daily" ? (
            <input
              ...
              className="px-3 py-2 border border-slate-200 rounded-lg text-sm font-bold text-slate-700 outline-none focus:border-blue-500 w-full sm:w-auto"
            />
          ) : (
            <input
              type="month"
          ...
          <button
            ...
            className="w-full sm:w-auto bg-slate-800 hover:bg-slate-900 text-white px-4 py-2 rounded-lg flex items-center justify-center gap-2 font-bold shadow-md transition-colors whitespace-nowrap"
          >
      ...
      <div className="bg-white p-4 sm:p-8 md:p-12 rounded-2xl md:rounded-2xl shadow-sm md:shadow-lg border border-slate-200 text-slate-800 print:p-0 print:m-0 print:shadow-none print:border-none">
        <div className="border-b-2 border-slate-800 pb-4 sm:pb-6 mb-6 sm:mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
          <div>
            <h1 className="text-xl sm:text-3xl font-black text-slate-900 uppercase tracking-tight">
              TBM Construction Report
            </h1>
            <p className="text-slate-500 font-medium mt-1 uppercase tracking-widest text-xs sm:text-sm">
              {reportType === "daily" ? "Daily Progress" : "Monthly Summary"}
            </p>
          </div>
          <div className="text-left sm:text-right">
            <div className="text-sm sm:text-lg font-bold text-slate-800 bg-slate-100 px-3 py-1 rounded inline-block">
              {displayDateStr}{" "}
              {reportShift !== "All" && (
                <span className="text-blue-600 ml-1">({reportShift})</span>
              )}
            </div>
            <div className="text-[10px] sm:text-xs text-slate-500 font-medium mt-2">
              {projectInfo.tbmNo} | {projectInfo.location}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4 mb-6 sm:mb-8">
          <div className="bg-slate-50 p-3 sm:p-4 rounded-xl border border-slate-200">
            <div className="text-[9px] sm:text-[10px] uppercase font-bold text-slate-500 mb-1">
              Rings Installed
            </div>
            <div className="text-lg sm:text-2xl font-black text-emerald-600">
              {summary.permCount}{" "}
              <span className="text-[10px] sm:text-xs text-slate-400 font-bold ml-1">
                Perm.
              </span>
            </div>
            <div className="text-[9px] sm:text-xs font-medium text-slate-500 mt-1">
              + {summary.tempCount} Temp (Total: {summary.totalSegments})
            </div>
          </div>
          <div className="bg-slate-50 p-3 sm:p-4 rounded-xl border border-slate-200">
            <div className="text-[9px] sm:text-[10px] uppercase font-bold text-slate-500 mb-1">
            ...
            <div className="text-[9px] sm:text-xs font-bold text-slate-500 mt-1">
              ดินขุดรวม: {summary.totalSoilVol} m³
            </div>
          </div>
          <div className="bg-slate-50 p-3 sm:p-4 rounded-xl border border-slate-200">
            <div className="text-[9px] sm:text-[10px] uppercase font-bold text-slate-500 mb-1">
            ...
            <div className="text-lg sm:text-2xl font-black text-indigo-600">
              {summary.totalGroutVol}{" "}
              <span className="text-[10px] sm:text-xs text-slate-400 font-bold ml-1">
                m³
              </span>
            </div>
            <div className="text-[9px] sm:text-xs font-medium text-slate-500 mt-1">
              {summary.uniqueGroutedRings} Rings Grouted
            </div>
          </div>
          <div className="bg-slate-50 p-3 sm:p-4 rounded-xl border border-slate-200">
            <div className="text-[9px] sm:text-[10px] uppercase font-bold text-slate-500 mb-1">
            ...
            <div
              className={`text-lg sm:text-2xl font-black ${
                parseFloat(summary.avgGroutRatio) > 150
                  ? "text-purple-500"
                  : parseFloat(summary.avgGroutRatio) >= 100
                  ? "text-emerald-500"
                  : "text-red-500"
              }`}
            >
              {summary.avgGroutRatio}{" "}
              <span className="text-[10px] sm:text-xs font-bold ml-1">%</span>
            </div>
            <div className="text-[9px] sm:text-xs font-medium text-slate-500 mt-1">
              Efficiency
            </div>
        ...
        <div className="mb-6 sm:mb-8 overflow-x-auto">
          <h3 className="text-xs sm:text-sm font-bold text-slate-800 uppercase tracking-widest border-l-4 border-emerald-500 pl-2 mb-3 sm:mb-4">
            Segment Installation Logs
          </h3>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredSegments.length > 0 ? (
                [...filteredSegments].reverse().map((r, i) => (
                  <tr key={`${r.id}-${i}`} className="hover:bg-slate-50">
                    <td className="py-2 px-2 sm:px-3">
                      ...
                      <div className="text-[9px] sm:text-[10px] text-slate-500 mt-1 font-mono">
                        <span className="font-bold">Excav:</span>{" "}
                        {formatDisplayTime(r.excavStartTime)} -{" "}
                        {formatDisplayTime(r.excavEndTime)}
                      </div>
                      <div className="text-[9px] sm:text-[10px] text-slate-500 mt-0.5 font-mono">
                        <span className="font-bold">Inst:</span>{" "}
                        {formatDisplayTime(r.installStartTime || r.startTime)} -{" "}
                        {formatDisplayTime(r.installEndTime || r.endTime)}
                      </div>
                    </td>
                    <td className="py-2 px-2 sm:px-3 font-bold text-slate-800">
                      <span
                        className={
                          r.installType === "Temporary" ? "text-amber-600" : ""
                        }
                      >
                        {r.ringNo}
                      </span>
                      {r.installType === "Temporary" && (
                        <span className="text-amber-500 ml-2 font-bold text-[8px] sm:text-[9px]">
                          (Temp)
                        </span>
                      )}
                      {r.status === "In Progress" && (
                        <span className="text-orange-500 ml-2 font-bold text-[8px] sm:text-[9px]">
                          (In Progress)
                        </span>
                      )}
                    </td>
                    <td className="py-2 px-2 sm:px-3">
                      {r.typeRing} - K{r.keyPos}
                    </td>
                    <td className="py-2 px-2 sm:px-3 text-slate-500 font-mono">
                      {r.startCH}
                    </td>
                    <td className="py-2 px-2 sm:px-3 text-slate-800 font-bold font-mono">
                      {r.finishCH}
                    </td>
                    <td className="py-2 px-2 sm:px-3 text-right">
                      ...
                      <div className="text-[9px] sm:text-[10px] text-amber-600 font-bold mt-0.5">
                        {r.soilVolume || calculateSoilVolume(r.length)} m³
                      </div>
                    </td>
                  </tr>
                ))
                ...
                <tr>
                  ...
                  >
                    No segment data for this period.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="mb-6 sm:mb-8 overflow-x-auto">
          <h3 className="text-xs sm:text-sm font-bold text-slate-800 uppercase tracking-widest border-l-4 border-blue-500 pl-2 mb-3 sm:mb-4">
            Primary Grout Logs
          </h3>
          <table className="w-full text-[10px] sm:text-xs text-left border-collapse whitespace-nowrap">
            <thead className="bg-slate-100 text-slate-600 border-y border-slate-300">
                  ...
                  <tr key={`${r.id}-${i}`} className="hover:bg-slate-50">
                    <td className="py-2 px-2 sm:px-3 font-bold text-slate-800">
                      ...
                      {r.groutPass === "Re-Grout" && (
                        <span className="text-purple-600 ml-2 font-bold text-[8px] sm:text-[9px]">
                          (Re-Grout)
                        </span>
                      )}
                    </td>
                    <td className="py-2 px-2 sm:px-3 text-slate-500">
                      {r.excavRing}
                    </td>
                    <td className="py-2 px-2 sm:px-3">
                      <div>K{r.key}</div>
                      <div className="flex gap-0.5 flex-wrap mt-0.5">
                        {Object.entries(r.positions || {}).map(
                          ([pos, active]) =>
                            active && (
                              <span
                                key={pos}
                                className="px-1 py-0.5 bg-blue-50 text-blue-700 text-[8px] sm:text-[9px] rounded border border-blue-100 font-bold leading-none"
                              >
                                {pos}
                              </span>
                            )
                    ...
                    <td className="py-2 px-2 sm:px-3 text-right text-slate-500">
                      {r.partA}
                    </td>
                    <td className="py-2 px-2 sm:px-3 text-right text-slate-500">
                      {r.partB}
                    </td>
                    <td className="py-2 px-2 sm:px-3 text-right font-bold text-slate-700">
                      {r.total.toFixed(2)}
                    </td>
        ...
        {summary.allRemarks.length > 0 && (
          <div>
            <h3 className="text-xs sm:text-sm font-bold text-slate-800 uppercase tracking-widest border-l-4 border-orange-500 pl-2 mb-3 sm:mb-4">
              Issues & Remarks
            </h3>
            <ul className="space-y-2 text-xs sm:text-sm text-slate-700">
              {summary.allRemarks.map((rem, i) => (
                <li
                  key={i}
                  className="bg-orange-50/50 p-2 sm:p-3 rounded-lg border border-orange-100 flex flex-col sm:flex-row gap-1 sm:gap-3"
                >
...
const ShiftReportView = ({
  ...
  const existingReport = useMemo(() => {
    return shiftReports.find(
      (r) => formatDisplayDate(r.date) === meta.date && r.shift === meta.shift
    );
  ...
  const autoResult = useMemo(() => {
    const shiftSegs = deduplicatedSegments.filter(
      (r) =>
    ...
    if (shiftSegs.length > 0) {
      const sorted = [...shiftSegs].sort(
        (a, b) => getRingNumeric(a.ringNo) - getRingNumeric(b.ringNo)
      );
      const startSta = sorted[0].startCH;
      const finishSta = sorted[sorted.length - 1].finishCH;
      const numRings = sorted.length;
      const totalDist = sorted
        .reduce((sum, r) => sum + parseFloat(r.length || 0), 0)
        .toFixed(2);
  ...
  const handleSaveToCloud = async () => {
    ...
    try {
      await apiCall(
        existingReport ? "updateShiftReport" : "addShiftReport",
        payload
      );
      ...
      if (existingReport) {
        setShiftReports((prev) =>
          prev.map((r) => (r.id === payload.id ? savedRecord : r))
        );
      } else {
  ...
  const displayEvents = useMemo(() => {
    ...
    const filteredSegs = deduplicatedSegments.filter(
      (r) => formatDisplayDate(r.date) === meta.date && r.shift === meta.shift
    );
    ...
    filteredSegs.forEach((rec) => {
      ...
      const instStart = formatDisplayTime(
        rec.installStartTime || rec.startTime
      );
  ...
  const activityCategories = [
    ...
    {
      group: "Delay Activities",
      items: [
        "Locomotive / Rail System",
        "Survey",
        "Power Supply",
        "TBM Equipment",
        "Clean Area",
    ...
    {
      group: "TBM Service / Maintenance",
      items: ["Cleaning Belt conveyor", "Service / Maintenance", "Other 3"],
    },
  ];
  ...
  const getBarColorClasses = (groupIndex) => {
    switch (groupIndex) {
      case 0:
        return "bg-stripe-blue border-blue-500 text-blue-900";
      case 1:
        return "bg-stripe-red border-red-500 text-red-900";
      case 2:
        return "bg-stripe-green border-green-500 text-green-900";
      default:
  ...
  const addEvent = (activityName) => {
    if (!newEvent.start || !newEvent.end)
      return alert("กรุณาระบุเวลาเริ่มและสิ้นสุด");
  ...
  const deleteEvent = (activityName, id) => {
    const updatedEvents = { ...events };
    updatedEvents[activityName] = updatedEvents[activityName].filter(
      (ev) => ev.id !== id
    );
    setEvents(updatedEvents);
  };
    ...
    <div className="max-w-6xl mx-auto font-sans text-sm pb-24 animate-fade-in">
      <div className="mb-4 bg-white p-4 rounded-xl shadow-sm flex flex-col sm:flex-row justify-between items-center gap-4 no-print border border-slate-200">
        ...
        <div className="flex w-full sm:w-auto gap-2">
          <button
            ...
            disabled={isSaving}
            className="flex-1 sm:flex-none bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-colors shadow-sm font-bold justify-center"
          >
            {isSaving ? (
          ...
          <button
            onClick={handlePrint}
            className="flex-1 sm:flex-none bg-slate-800 hover:bg-slate-900 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-colors shadow-sm font-bold justify-center"
          >
            <Printer size={18} /> Print PDF
          </button>
        </div>
      ...
      <div className="bg-white border border-gray-300 shadow-sm print:shadow-none print:border-none rounded-xl overflow-hidden print:rounded-none">
        <div className="p-6 border-b-2 border-black print:p-2 text-center bg-white">
          <h2 className="text-xl print:text-lg font-bold mb-1">
            บันทึกการทำงานการขุดเจาะอุโมงค์
          </h2>
          <p className="font-semibold text-gray-700 print:text-black text-sm mb-1">
            โครงการงานก่อสร้างอุโมงค์ระบายน้ำคลองเปรมประชากรจากคลองบางบัวลงสู่แม่น้ำเจ้าพระยา
          </p>
          <div className="flex flex-col sm:flex-row justify-center gap-3 sm:gap-6 text-xs sm:text-sm mt-3 text-gray-800 print:text-black items-center">
            <p>
              <strong>ผู้ว่าจ้าง :</strong> สำนักการระบายน้ำ กรุงเทพมหานคร
            </p>
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center bg-white px-2 py-0.5 rounded border border-gray-300 shadow-sm print:shadow-none print:border-gray-400">
                <span className="text-[#004a80] font-black tracking-tighter border-r border-gray-300 pr-2 text-xs">
                  TEAM GROUP
                </span>
            ...
            <p>
              <strong>ผู้รับจ้าง :</strong> กิจการร่วมค้า
              ไอทีดี-เอ็นดับเบิลยูอาร์
            </p>
          </div>
        </div>
        ...
        <div className="p-4 print:p-2 border-b-2 border-black flex flex-wrap gap-4 items-center bg-gray-50 print:bg-white text-xs sm:text-sm">
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              className="border border-slate-300 p-1.5 rounded-md print:border-none print:bg-transparent text-gray-700 outline-none"
            />
          </div>
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              value={meta.tbmNo}
              onChange={handleMetaChange}
              className="border border-slate-300 p-1.5 rounded-md w-24 print:border-none print:bg-transparent outline-none"
            />
          </div>
          <div className="flex items-center gap-2 flex-grow">
            ...
            <input
              ...
              value={meta.location}
              onChange={handleMetaChange}
              className="border border-slate-300 p-1.5 rounded-md w-full print:border-none print:bg-transparent outline-none"
            />
          </div>
              ...
              {activityCategories.map((cat, cIdx) => (
                <React.Fragment key={cIdx}>
                  <tr className="bg-gray-200">
                    <td
                      colSpan={14}
                      className="border border-black p-1.5 font-bold pl-2"
                    >
                      {cat.group}
                    </td>
                  </tr>
                  {cat.items.map((item, iIdx) => {
                    ...
                    return (
                      <tr key={`${cIdx}-${iIdx}`} className="group h-[34px]">
                        ...
                        <td
                          colSpan={12}
                          className="border border-black p-0 relative"
                        >
                          <div className="absolute inset-0 flex pointer-events-none">
                            {currentHours.map((h, idx) => (
                              <div
                                key={idx}
                                className={`h-full w-[8.333%] ${
                                  idx < 11
                                    ? "border-r border-gray-200 print:border-gray-300"
                                    : ""
                                }`}
                              />
                            ))}
                          ...
                          <div className="absolute inset-y-[4px] inset-x-0">
                            {(displayEvents[item] || []).map((ev, index) => {
                              const { left, width } = calculateBarStyles(
                                ev.start,
                                ev.end,
                                meta.shift
                              );
                              ...
                              return (
                                <div
                                  ...
                                  style={{ left, width }}
                                  onClick={() =>
                                    !ev.isAuto && setActiveModal(item)
                                  }
                                  title={`${ev.start} - ${ev.end}`}
                                >
                                  <span className="bg-white/90 px-1 rounded-sm shadow-sm">
                                    {ev.label}
                                  </span>
                                </div>
                              );
              ...
              {Object.keys(manpower).map((role) => (
                <div key={role} className="flex flex-col">
                  ...
                  {renderInput(
                    ...
                    "0",
                    "number",
                    "text-center bg-gray-50 p-1.5 rounded"
                  )}
                </div>
              ))}
            </div>
              ...
              <div className="flex items-center justify-between border-b border-dashed border-gray-300 pb-1">
                ...
                <div className="w-24">
                  {renderInput(
                    ...
                    "",
                    "number",
                    "text-right font-bold text-blue-800"
                  )}
                </div>
              </div>
              <div className="flex items-center justify-between border-b border-dashed border-gray-300 pb-1">
        ...
        <div className="p-4 print:p-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 text-center text-xs print:text-[11px] min-h-[140px] bg-white">
          {[
            "ผู้บันทึก (ผู้รับจ้าง)",
            "วิศวกร (ผู้รับจ้าง)",
            "วิศวกร (กลุ่มบริษัทที่ปรึกษา)",
            "ผู้ควบคุมงาน สำนักการระบายน้ำ",
          ].map((role, idx) => (
            <div key={idx} className="flex flex-col justify-end space-y-4">
              ...
              <div className="border-b border-black w-3/4 mx-auto pb-6 relative mt-4">
                <input
                  type="text"
                  className="absolute bottom-0 left-0 w-full text-center bg-transparent outline-none grid-input font-bold"
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 no-print">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-[450px] max-w-[90%] transform transition-all">
            <div className="flex justify-between items-center border-b pb-3 mb-4">
              ...
              <button
                onClick={closeModal}
                className="text-gray-400 hover:text-red-500 bg-slate-100 hover:bg-red-50 rounded-full p-1 transition-colors"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="mb-6">
              ...
              <input
                ...
                placeholder="เช่น 108 หรือ K-14"
                className="w-full border border-slate-300 rounded-lg p-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
            ...
            {displayEvents[activeModal] &&
              displayEvents[activeModal].length > 0 && (
                <div>
                  <h4 className="font-bold text-xs uppercase tracking-widest mb-2 text-slate-500 border-t border-slate-100 pt-4">
                    รายการที่บันทึกไว้แล้ว
                  </h4>
                  <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
                    {displayEvents[activeModal].map((ev, index) => (
                      <div
                        key={`${ev.id}-${index}`}
                        className={`flex justify-between items-center bg-slate-50 p-2.5 rounded-lg text-sm border ${
                      ...
                      >
                        <span>
                          {ev.start} - {ev.end}{" "}
                          <strong className="ml-2 text-blue-700 bg-blue-100 px-1.5 py-0.5 rounded">
                            [{ev.label}]
                          </strong>
                        </span>
                        {ev.isAuto ? (
                          <span className="text-[10px] text-emerald-600 bg-emerald-50 px-2 py-1 rounded font-bold border border-emerald-100">
                            Auto (System)
                          </span>
                        ) : (
                          <div className="flex items-center gap-1">
                            ...
                            <button
                              onClick={() => deleteEvent(activeModal, ev.id)}
...
const PrimaryGroutApp = () => {
      ...
      if (GAS_URL !== "YOUR_WEB_APP_URL_HERE" && GAS_URL.startsWith("http")) {
        try {
          ...
          if (textData.trim().startsWith("<"))
            throw new Error("Received HTML error.");
          ...
          if (result.status === "success") {
            ...
            const parsedShiftReports = (result.shiftReports || []).map(
              (sr) => ({
                ...sr,
  ...
  const liveHeaderStatus = useMemo(() => {
    ...
    if (lastSeg.status === "In Progress") {
      if (lastSeg.excavStartTime && !lastSeg.excavEndTime)
        return {
          text: `กำลังขุดเจาะ ${lastSeg.ringNo}`,
          color: "bg-amber-500",
          icon: <AlertCircle size={12} />,
        };
      if (lastSeg.excavEndTime && !lastSeg.installStartTime)
        return {
          text: `ขุดเสร็จ รอประกอบ ${lastSeg.ringNo}`,
          color: "bg-slate-500",
          icon: <Clock size={12} />,
        };
      if (lastSeg.installStartTime && !lastSeg.installEndTime)
        return {
          text: `กำลังประกอบ ${lastSeg.ringNo}`,
          color: "bg-emerald-500",
          icon: <Activity size={12} />,
        };
      return {
        text: `กำลังทำงาน ${lastSeg.ringNo}`,
        color: "bg-blue-500",
        icon: <Activity size={12} />,
      };
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 font-sans text-slate-800 selection:bg-blue-100 overflow-x-hidden">
      ...
      <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-slate-200/60 shadow-sm transition-all no-print">
        <div className="max-w-7xl mx-auto px-4 py-3 flex flex-col md:flex-row justify-between items-center gap-3">
          <div className="flex items-center gap-3 w-full md:w-auto">
            <div
              className={`p-2 sm:p-2.5 rounded-xl text-white shadow-lg transition-colors bg-slate-800`}
            >
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {liveHeaderStatus && (
                  <div
                    className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-bold text-white shadow-sm animate-pulse ${liveHeaderStatus.color}`}
                  >
              ...
              <div className="flex items-center gap-1 text-[9px] sm:text-[10px] text-slate-500 font-medium mt-0.5">
                ...
                <input
                  ...
                  onChange={handleProjectInfoChange}
                  list="locations-list"
                  className="bg-transparent border-none p-0 focus:ring-0 w-48 sm:w-64 outline-none cursor-pointer placeholder-slate-400"
                  placeholder="สถานที่..."
                />
          ...
          {activeTab !== "overview" && activeTab !== "shift_report" && (
            <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 w-full md:w-auto">
              <button
                ...
                className={`flex-1 md:flex-none px-4 py-2 rounded-lg text-[10px] sm:text-xs font-bold transition-all ${
      ...
      <main className="px-2 sm:px-4 py-4 sm:py-8 max-w-7xl mx-auto print:p-0 print:m-0">
        {loadError && (
          <div className="mb-4 bg-red-50 text-red-700 p-4 rounded-xl text-center no-print">
            {loadError}
          </div>
        )}
        ...
        {activeTab === "record" && currentModule === "grout" && (
          <GroutRecordView
            ...
            groutRecords={groutRecords}
            setGroutRecords={setGroutRecords}
            segmentRecords={segmentRecords}
            setCurrentModule={setCurrentModule}
            setActiveTab={setActiveTab}
          />
        ...
        {activeTab === "record" && currentModule === "segment" && (
          <SegmentRecordView
            ...
            handleProjectInfoChange={handleProjectInfoChange}
            segmentRecords={segmentRecords}
            setSegmentRecords={setSegmentRecords}
            setCurrentModule={setCurrentModule}
            setActiveTab={setActiveTab}
          />
        )}
        ...
        {activeTab === "shift_report" && (
          <ShiftReportView
            ...
            setShiftReports={setShiftReports}
          />
        )}
      </main>
      ...
      <nav className="fixed bottom-4 sm:bottom-6 left-1/2 transform -translate-x-1/2 z-40 no-print w-[95%] sm:w-auto max-w-2xl">
        <div className="flex items-center justify-between sm:justify-center gap-1 bg-slate-900/95 backdrop-blur-xl border border-white/10 shadow-2xl p-1.5 rounded-full overflow-x-auto hide-scrollbar">
          <button
            ...
            className={`flex-none flex items-center justify-center gap-1.5 px-3 sm:px-4 py-2.5 rounded-full text-[10px] sm:text-xs font-bold transition-all ${
              activeTab === "overview"
